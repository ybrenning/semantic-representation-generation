...
def parse_varfreeLF(sent, varfreeLF):
    ...
    for i, c in enumerate(lf):
        if c == '(':
            ...
            # PCRE pattern regex that splits the string based on commas
            # outside all parenthesis
            arg_substring_list = [
                x for x in regex.split(
                    r"(\((?:[^()]++|(?1))*\))(*SKIP)(*F)|,", arg
                ) if x
            ]
            # match any characters as few as possible that are followed
            # by an opening parenthesis
            pattern = '.+?(?=\()'
            match = [
                re.search(pattern, x).group(0).strip()
                if re.search(pattern, x) else x.strip()
                for x in arg_substring_list
            ]
...
def replace_tokens_with_indexes(tokens, lf):
    """Iteratively replace tokens in the variable-free logical form with their
    respective indexes in the sentence to address issues with identical verbs.
    Datasets generated using `alto-2.3.9-SNAPSHOT-all.jar` avoid this issue
    by filtering out sentences with duplicate lemmas during the generation
    process.
    """
    ...
    for word in words_in_lf:
        if (
            word in tokens[token_index:]
            and word not in ("in", "on", "beside")
        ):
...
def get_idx_varfreeLF(sent, lf):
    ...
    lemma_tokens = [
        verbs_lemmas[token]
        if token in verbs_lemmas.keys() else token
    ...
    while bool(re.search(r'= (\* )?[a-z]', idx_varfreeLF)):
        ...
        if c > 30:
            raise Exception(
                "too much iteration in getting idx_varfreeLF:"
                "\n {} \n compared to original:\n {}".format(idx_varfreeLF, lf)
            )
    varfreeLF = [
        lemma_tokens[int(token)]
        if token.isnumeric() else token
        for token in idx_varfreeLF.split()
    ]
    if " ".join(varfreeLF) != lf:
        raise Exception(
            "Error in parsing varfreeLF:"
            "\n {} \n compared to original:"
...
def varfree_to_cogs_lf(sent, varfreeLF):
    ...
    if len(tokens_list) == 1:
        raise Exception(
            "The converter doesn't support primitive logical forms, "
            "please use 'generate_primitives.py' script"
        )
    ...
    definite_noun_list = [False] + [
        True if token.lower() == "the" else False for token in tokens_list
    ]
    indefinite_noun_list = [False] + [
        True if token.lower() == "a" else False for token in tokens_list
    ]
    defini_nouns = []
    main_lf = []
