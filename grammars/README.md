...
For example, if we make modifications to our base grammar (`combined-grammars.irtg`), we must first `include` it in the final line of `specify_grammar.irtg`:
...
{% include 'slog-rec_pp_2.irtg' %}
```

Then use the preprocess script to render the contents:

```bash
$ python3 cogs-preprocess.py specify_grammar.irtg > preprocessed-slog-rec_pp_2.irtg
```
...
```bash
$ python3 irtg2ebnf.py preprocessed-slog-rec_pp_2.irtg
```

The resulting English grammar always has the same name as the original file, so `preprocessed-slog-rec_pp_2.irtg.ebnf`
